...
import { PlaneInfo } from "../component/Plane";
/**
 * 键盘移动
 * @param x 初始化 x 坐标值
 * @param y 初始化 y 坐标值
...
export const useKeyboardMove = ({ x, y, speed }) => {
  ...
  const handleTicker = () => {
    ...
    // 我方飞机 moveY 和 moveX 关在游戏界面内
    if (moveY.value <= 0) {
      moveY.value = 0;
    }
    if (moveY.value + PlaneInfo.height >= stage.height) {
      moveY.value = stage.height - PlaneInfo.height;
    }
    ...
    if (moveX.value + PlaneInfo.width > stage.width) {
      moveX.value = stage.width - PlaneInfo.width;
    }
